...
Effectively re-implements OpenAI's Image GPT model:
  https://openai.com/blog/image-gpt/
Getting GPT to model images instead of text, but using a near identical model.
It's truly quite remarkable that a single model can agnostically do a great job
modeling whatever data you give it: text, images, or whatever else.

At the end of the day it is just a sequence of integers.
Notice that unlike models like PixelCNN++ etc, this model
knows nothing at all about the spatial layout of the pixels
and has to learn the appropriate positional embeddings
that reflect the spatial topology of the data.
"""
...
if __name__ == '__main__':
    ...
    # Images are represented as array of size (height, width, 3),
    # where is the RGB values, each is a uint8 in range 0..255.
    # In CIFAR-10 for example, the height and width are both 32.
    #
    # **naive strategy**
    # Now, to feed images into GPT we have to somehow turn every image into a sequence of integers.
    # Since each image is 32\*32\*3 = 3072 uint8s, in principle we could just flatten each image
    # out into a 3072-long sequence of numbers from 0..255 and train GPT on that.
    # Note that we are free to feed this into GPT in any random arbitrary order,
    # as long as the encoding order is fixed for all images. The problem with this is that
    # GPT gets very expensive as you grow the sequence size, since each new predicted integer
    # is a function of all previously predicted integers in the sequence, and the attention
    # inside the Transformer modules gets very expensive.
    #
    # **k-means codebook strategy**
    # Instead, the Image GPT strategy is to encode every individual RGB pixel into
    # a codebook of 512 entries, which we train via the k-means clustering algorithm.
    # This way, we only have a 32\*32 = 1024-long sequence, but now of integers
    # in the range 0..511. This is a net saving in compute because we're "shrunk"
    # the sequence length by a factor of 3. On the other hand, our token encoding
    # embedding parameters grow a bit in size, as well as the size of the Softmax
    ...
    def pluck_rgb(x):
        return torch.from_numpy(np.array(x)).view(32 * 32, 3)[torch.randperm(32 * 32)[:5], :]
    ...
    # The images above look relatively reasonable, so our 512-sized codebook is enough to reasonably
    # re-represent RGB values. Ok cool. So now every image is just a 1024-long sequence
    # of numbers between 0..511. Time to train a GPT.
    train_dataset = ImageDataset(train_data, C)
    test_dataset = ImageDataset(test_data, C)

    # one example image flattened out into integers
